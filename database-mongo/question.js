...
function addAnswers() {
  ...
  const stream = fs.createReadStream(filename)
  .pipe(csv(['answer_id', 'question_id', 'body', 'date', 'answerer_name', 'asker_email', 'reported', 'helpfulness']))
  .on('data', (data) => {
      if(data.reported === 'f') {
        data.reported = 0;
...
function addHelpfull() {
  let filename = path.join(__dirname, 'answershelpful.csv')
  let errors = [];
  let count = 0;
  const stream = fs.createReadStream(filename)
  .pipe(csv(['answer_id', 'helpfulness']))
  .on('data', (data) => {
      stream.pause()
      Question.findOneAndUpdate({"answers.answer_id": data.answer_id}, {"$set": {"answers.$.helpfulness": data.helpfulness}})
        .then(data => {
          count++;
          stream.resume()
        })
  ...
  .on('end', () => {
    console.log('end, total of:', count);
  });
}
